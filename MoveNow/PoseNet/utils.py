...
def _process_input(source_img, scale_factor=1.0, output_stride=16):
    ...
    #grayscale
    # input_img = cv2.cvtColor(input_img, cv2.COLOR_BGR2GRAY).astype(np.float32)
    # input_img = np.expand_dims(input_img, 2)
    # input_img = np.insert(input_img, 1, [input_img[:,:,0]] * 2, axis = 2)
    input_img = input_img * (2.0 / 255.0) - 1.0
    input_img = input_img.transpose((2, 0, 1)).reshape(1, 3, target_height, target_width)
    return input_img, source_img, scale


def read_cap(cap, ip_webcam = False, scale_factor=1.0, output_stride=16):
    if ip_webcam:
        img = cv2.imdecode(cap, -1) #-1 keep origin, 0 grayscale 1080, 1920
    else:
