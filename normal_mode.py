...
target_poses = [pose.path for pose in os.scandir(current_path + '/players/target_posedata/json') if pose.path.endswith('.json')]
...
def Normal_Mode(args, posenet, output_video = None):
    ...
    if args['imwrite']:
        ...
        cv2.imwrite(f"{current_path}/result_images/{savetime}.png", result_img_)
...
def instruction_normal(cv2_img):
    ...
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/instruction_normal.png', 0.2305, 0.2914, 0.7719, 0.70)
...
def display_result(cv2_img, results, perfect_pct, good_pct, poor_pct, missing_pct, times):
    ...
    """labels"""
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/seperator.png', 0.0764, 0.1278, 0.1092, 0.6153)
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/statistics.png', 0.1031, 0.0402, 0.2852, 0.1194)
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/perfect.png', 0.0055, 0.1694, 0.0861, 0.2333)
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/good.png', 0.0055, 0.2763, 0.0961, 0.3402) #0.0639
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/poor.png', 0.0055, 0.3832, 0.0961, 0.4471)
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/missing.png', 0.0055, 0.4901, 0.0861, 0.554)
    overlay = overlay_transparent(overlay, img, tlx, tly, (brx - tlx, bry - tly))
    
    """display results"""
    if perfect_pct != 0:
        img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/result_perfect.png', 0.0977, 0.1639, 0.2945, 0.2333)
    ...
    if good_pct != 0:
        img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/result_good.png', 0.0977, 0.2708, 0.2945, 0.3402)
        diff_good = tlx + int((brx - tlx) * good_pct / 20 * times)
        overlay = overlay_transparent(overlay, img, tlx, tly, (diff_good - tlx, bry - tly))
    if poor_pct != 0:
        img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/result_poor.png', 0.0977, 0.3777, 0.2945, 0.4471)
        diff_poor = tlx + int((brx - tlx) * poor_pct / 20 * times)
        overlay = overlay_transparent(overlay, img, tlx, tly, (diff_poor - tlx, bry - tly))
    if missing_pct != 0:
        img, tlx, tly, brx, bry = find_box(overlay, current_path + '/UI_images/result_missing.png', 0.0977, 0.4846, 0.2945, 0.554)
